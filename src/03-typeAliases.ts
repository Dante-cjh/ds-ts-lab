...
// Extract all Friend objects from BuddyList
function getBuddyListFriends(list: BuddyList): Friend[] {
    return list.members.reduce((acc: Friend[], buddy: Buddy) => {
        // Check if Buddy is of type Friend
        if ("phone" in buddy) {
            acc.push(buddy as Friend); // Convert Buddy to Friend and add to accumulator
        }
        return acc;
    }, []);
}

// Testing the getBuddyListFriends Function
